  ...
  <body id="contentContainer" class="container bg-dark" >
    ...
    <div class="row">
      <div class="btn-group">
        ...
        ?>
        <?php
          $query_countreqp = mysqli_query($con,"SELECT count(*) FROM health_record WHERE health_iscomplete = 'no' AND health_type='pregnant'");
          $result_countreqp = mysqli_fetch_array($query_countreqp);
          $reqcountp = $result_countreqp[0];
        ?>
        ...
        <a href="opsviewer_c.php" class="btn btn-secondary active" aria-current="page">Consultant Parking <span class="badge bg-dark rounded-pill"><?php echo $reqcountc; ?></span></a>
      ...
      <table class="table table-striped-columns shadow rounded" width="98%" style="border-collapse: collapse; font-size: 13px">
        <thead>
          <tr class="">
            ...
            <td class="bg-dark bg-gradient shadow-sm fw-bold" style="width:100px;color:#E8B820"><center>Doctor Code</center></td>
        ...
        <tbody>
          <?php
            $searchQuery = mysqli_query($con,"SELECT * FROM user
            WHERE user_type='3' AND user_isactive='yes'");
            $count = 0;
            while($searchResult = mysqli_fetch_array($searchQuery))
            {
              $user_id = $searchResult['user_id'];
